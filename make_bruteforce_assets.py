...
base = Path(r'O:\Descargas\hackaton\bruteforce_assets')
(base / 'pages').mkdir(parents=True, exist_ok=True)
(base / 'materials').mkdir(parents=True, exist_ok=True)

files = {}

files['pages/bruteforce-lab.html'] = '''<div class="cuhv-page cuhv-reveal">
  <section class="cuhv-hero">
    <div class="cuhv-hero-copy">
      <span class="cuhv-kicker">Fuerza bruta controlada</span>
      <h1>Autenticación débil en servicios de laboratorio</h1>
      <p>Estos retos muestran dos casos clásicos en entorno académico controlado: autenticación básica con credenciales previsibles y formularios de login con verificación pobre. La intención es practicar enumeración de parámetros y uso preciso de <code>hydra</code>, no automatización agresiva contra sistemas reales.</p>
    </div>
    <figure class="cuhv-photo-card"><img src="https://commons.wikimedia.org/wiki/Special:Redirect/file/Commons-emblem-padlock.svg" alt="Candado"><figcaption>Autenticación y sesión en entorno controlado. Fuente externa: Wikimedia Commons.</figcaption></figure>
  </section>
  <section class="cuhv-section cuhv-reveal">
    <div class="cuhv-grid cuhv-grid-2">
      <article class="cuhv-card"><h3>Qué aprender</h3><ul class="cuhv-list"><li>Distinguir Basic Auth de formularios POST.</li><li>Identificar usuario, ruta y criterio de éxito o fallo.</li><li>Usar <code>hydra</code> con parámetros mínimos y baja complejidad.</li></ul></article>
      <article class="cuhv-card"><h3>Qué no hacer</h3><ul class="cuhv-list"><li>No salgas del alcance del laboratorio.</li><li>No dispares pruebas ruidosas fuera de los endpoints publicados.</li><li>No asumas el formato del login sin revisar antes headers, HTML y respuesta.</li></ul></article>
    </div>
  </section>
</div>
'''

files['materials/formulario_de_acceso/README.txt'] = 'Material auxiliar para el reto Formulario de acceso. El usuario válido de laboratorio es analista. La contraseña se encuentra dentro de un conjunto pequeño de candidatos deliberadamente débiles para práctica controlada.\n'
files['materials/formulario_de_acceso/usuario.txt'] = 'analista\n'
files['materials/formulario_de_acceso/claves.txt'] = 'laboratorio\nanalisis\ncuh2026\npassword\nwelcome\n'

files['deploy_bruteforce_remote.sh'] = '''#!/usr/bin/env bash
set -euo pipefail
BASE=/opt/cuh-ctf
...
mkdir -p "$CHALLENGES/acceso_por_defecto" "$CHALLENGES/formulario_de_acceso/materiales" "$SCRIPTS" "$PAGES"
...
## Propósito didáctico
Practicar fuerza bruta controlada con `hydra` sobre autenticación HTTP Basic en un entorno académico autorizado.

## Servicio
- URL: `http://45.55.49.111:8090/panel`
- Usuario válido: `operador`
- Contraseña débil: no se entrega en el enunciado

## Objetivo del alumno
- Identificar que el servicio usa Basic Auth
- Ajustar `hydra` con el módulo correcto
- Recuperar la credencial válida y leer la flag
EOF
...
- `hydra -l operador -P rockyou_pequeno.txt 45.55.49.111 http-get /panel`
EOF
...
printf 'codex2026\n' | sudo -S -p '' systemctl enable --now cuh-basic-auth.service
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if username == VALID_USER and password == VALID_PASS:
            self._send(200, f'<html><body><h1>Acceso concedido</h1><p>flag={FLAG}</p></body></html>')
        else:
            self._send(200, f'<html><body><h1>{FAIL}</h1><p>Intenta de nuevo.</p></body></html>')
...
mkdir -p "$CHALLENGES/formulario_de_acceso/materiales"
...
## Propósito didáctico
Practicar fuerza bruta controlada contra un formulario `POST` sencillo con `hydra` en Kali Linux.

## Servicio
- URL: `http://45.55.49.111:8091/`
- Ruta de login: `/login`
- Usuario válido: entregado en el material auxiliar

## Objetivo del alumno
- Revisar el HTML del formulario
- Identificar nombres de parámetros y texto de fallo
- Ajustar `hydra` con `http-post-form`
EOF
...
printf 'codex2026\n' | sudo -S -p '' systemctl daemon-reload
printf 'codex2026\n' | sudo -S -p '' systemctl enable --now cuh-form-login.service
...
cat > "$WORKDIR/sync_bruteforce.py" <<'PY'
...
SPECS = [
  {
    ...
    'type': 'standard',
    'state': 'visible',
    'description': 'Existe un panel HTTP de laboratorio en `http://45.55.49.111:8090/panel` dentro de este entorno académico controlado y autorizado. La autenticación es débil y suficiente para practicar fuerza bruta con herramientas estándar de Kali Linux. Antes de automatizar, identifica qué tipo de autenticación usa el servicio y luego prueba una lista razonable de contraseñas en el endpoint correcto.',
    'flag': 'CUH{la_fuerza_bruta_tambien_es_contexto}',
    'hints': ['Primero identifica si el servicio usa Basic Auth o formulario.', 'Si el usuario ya está definido, concentra la prueba en la contraseña.', 'Hydra puede resolver esto con el módulo HTTP adecuado.'],
    'attachment': None,
  },
  {
    'name': 'Formulario de acceso',
    ...
    'type': 'standard',
    'state': 'visible',
    'description': 'Se ha publicado un formulario de login de laboratorio en `http://45.55.49.111:8091/` dentro de este entorno académico controlado y autorizado. El material adjunto contiene una pista operativa suficiente para acotar la prueba. Antes de automatizar, revisa el formulario, identifica los nombres reales de los campos y determina cuál es la cadena de error que devuelve el sitio cuando el acceso falla.',
    'flag': 'CUH{hydra_tambien_necesita_precision}',
    'hints': ['No uses el módulo de Basic Auth: aquí hay un formulario POST.', 'Necesitas usuario, parámetros y cadena de fallo correctos.', 'Una wordlist pequeña es suficiente en este laboratorio.'],
    'attachment': '/opt/cuh-ctf/challenges/formulario_de_acceso/bruteforce_materiales.zip',
  },
]
app = create_app()
with app.app_context():
    for spec in SPECS:
        ...
        for h in Hints.query.filter_by(challenge_id=challenge.id).all():
            db.session.delete(h)
        db.session.flush()
        db.session.add(Flags(challenge_id=challenge.id, type='static', content=spec['flag'], data='case_insensitive'))
        for hint in spec['hints']:
            db.session.add(Hints(challenge_id=challenge.id, content=hint, cost=0))
        existing_files = ChallengeFiles.query.filter_by(challenge_id=challenge.id).all()
        for cf in existing_files:
            try:
                delete_file(cf.location)
            except Exception:
                pass
...
PY
docker cp "$WORKDIR/sync_bruteforce.py" "$CTFD_CONTAINER:/tmp/sync_bruteforce.py"
docker exec -e PYTHONPATH=/opt/CTFd "$CTFD_CONTAINER" python3 /tmp/sync_bruteforce.py
EOF
